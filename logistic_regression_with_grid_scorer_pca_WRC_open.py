...
#save the prices for white reality check
openp = df['<OPEN>'].copy() #if you trade at the open
close = df['<CLOSE>'].copy() #if you trade at the close
...
#    df_period = df.pct_change(periods=n)
#    name = 'mom_ret' + str(n)
...
#we DO NOT turn off scaling though returns are already mostly scaled (we are not using DUMMIES)
#do not use PCA() for dummies
scaler = StandardScaler(with_mean=True, with_std=True) #we kept the open, centering is not enough, scaling is needed
pca = PCA()
...
param_grid =  [{'pca__n_components':ncomponents_rs, 'logistic__C': c_rs, 'logistic__penalty': p_rs}]
...
print("the number of features kept was: ", best_model[1].n_components)

#plot the coefficients
importance = pd.DataFrame(zip(best_model[2].coef_.ravel().tolist(), list(range(1,np.size(best_model[2].coef_)+1))))
