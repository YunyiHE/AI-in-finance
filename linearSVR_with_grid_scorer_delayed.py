...
#We are going to shift the data for signal construction downwards (via shift(1)), 
#except the open (which we can take as given at time t)
#so as to be able to calculate the system returns 
#by multiplying the position at time t by the target returns at time t
#This is perhaps easier to grasp than what we have been doing:
#not shift down the data for signal contruction and
#calculating the system returns 
#by multiplying the position at time t-1 by the returns at time t 
...
#df['retFut2'] = df['<CLOSE>'].pct_change(1).shift(-2).fillna(0) #if you wait until the close to enter the trade
...
RESULTS
myscorer = make_scorer(information_coefficient, greater_is_better=True)
Out-of-sample: CAGR=0.00118164 Sharpe ratio=0.052598 maxDD=-0.100617 maxDDD=900 Calmar ratio=0.0117439  Rho=0.0267045 PVal=0.232585
p_value:
0.19579999999999997
"""
