...
lg = 3
df['vratio']=df['ret1'].rolling(100*lg).apply(lambda s: ff.vratio(s.values, lag=lg, cor='hom')[0]).fillna(0)
...
from sklearn.feature_selection import f_classif #ANOVA F-value between label/feature for classification tasks. (linear)
from sklearn.feature_selection import mutual_info_classif #Mutual information for a discrete target. (non linear)
...
def phi_k_selection_helper(x, y):
    dfc = pd.DataFrame({'y_true': y, 'y_pred': x})
    print("in selection helper")
    try:
        phi_k_corr = dfc.phik_matrix().iloc[1,0]  #the use of interval_cols=[] slows down the function too much
        phi_k_sig  = dfc.significance_matrix().iloc[1,0] #the use of interval_cols=[] slows down the function too much
        phi_k_p_val = 1 - ndtr(phi_k_sig) 
    except:
        phi_k_corr = 0
        phi_k_p_val = 0
...
def phi_k_select(X, y):
    ...
    for i in range(X.shape[1]):
         rho_arr[i], pval_arr[i] = phi_k_selection_helper(y,X[:, i])
...
selector = SelectPercentile(score_func= phi_k_select, percentile=50) #f_classif vs phi_k_select
#selector = SelectPercentile(score_func= f_classif, percentile=50) #f_classif vs phi_k_select

pipe = Pipeline([("scaler", scaler), ("select", selector), ("logistic", logistic)])
...
print("the number of features kept was: ", np.size(best_model[2].coef_))
