...
def select_scaler(x_train):
        ...
        df_filtered = df.select_dtypes(exclude=['uint8', 'int32'])
        df_scaled = pd.DataFrame(scaler.fit_transform(df_filtered))
        df_dummies = df.select_dtypes(include=['uint8', 'int32'])
...
pipe = Pipeline([("scaler", scaler), ("select_scaler", select_scaler), ("logistic", logistic)])
