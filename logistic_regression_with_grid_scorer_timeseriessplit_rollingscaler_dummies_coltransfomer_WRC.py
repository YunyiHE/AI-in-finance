...
def rolling_scaler(x_train, span=None):
        x_train = pd.DataFrame(x_train) #SEE NOTE
        df_rolling_std= x_train.rolling(span).std().fillna(1)
        df_rolling_mean = x_train.rolling(span).mean().fillna(0)
        df_centered = x_train.subtract(df_rolling_mean).fillna(0)
        df_scaled = df_centered.divide(df_rolling_std).fillna(0)
        df_scaled = df_scaled.replace([np.inf, -np.inf], np.nan).fillna(0)
        return df_scaled
#NOTE:
#Normally when ColumnTransformer and FunctionTransformer are used together 
#the pipeline expects x_train to be a dataframe.
#In this programm unexpectedly x_train got converted into an array (probably an effect of an "older" model) 
#and so we had to include this extra line to convert the x_train into a dataframe
...
# determine numerical features I want to scale
#SEE NOTE
#numerical_ix = x_train.select_dtypes(include=['float64', 'int64']).columns (*)
numerical_ix = [] #(**)
for i in x_train.select_dtypes(include=['float64', 'int64']).columns: #(***)
    numerical_ix.append(df.columns.get_loc(i)) #(****)
#NOTE:
#Normally when ColumnTransformer and FunctionTransformer are used together 
#the pipeline expects x_train to be a dataframe.
#In this programm unexpectedly x_train got converted into an array (probably an effect of an "older" model) 
#As a result, instead of using the line marked (*) which expects a dataframe
#I had to use the lines (**), (***) and (****) which expect an array instead 


# define the data preparation for the columns
t = [('rol', rolling_scaler, numerical_ix)]
col_transform = ColumnTransformer(transformers=t, remainder='passthrough')

logistic = LogisticRegression(max_iter=1000, solver='liblinear') 

pipe = Pipeline([("scaler", scaler), ("prep", col_transform), ("logistic", logistic)])
...
param_grid =  [{'prep__rol__kw_args':  spans_rs,'logistic__C': c_rs, 'logistic__penalty': p_rs}]
